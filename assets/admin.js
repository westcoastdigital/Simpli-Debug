jQuery(document).ready(function($) {
    ...
    if ($('.simpli-activity-wrap').length) {
        ...
        function loadActivityLogs() {
            ...
            $tbody.html('<tr class="simpli-loading-row"><td colspan="7" class="simpli-loading"><span class="spinner is-active"></span> Loading activity log...</td></tr>');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $tbody.html('<tr><td colspan="7" class="simpli-no-results">Error loading logs</td></tr>');
                    }
                },
                error: function() {
                    $tbody.html('<tr><td colspan="7" class="simpli-no-results">Error loading logs</td></tr>');
        ...
        function renderActivityLogs(logs) {
            ...
            if (logs.length === 0) {
                $tbody.html('<tr><td colspan="7" class="simpli-no-results">No activity logs found matching your filters.</td></tr>');
                return;
            }
            ...
            $.each(logs, function(index, log) {
                ...
                var typeLabel = log.log_type.charAt(0).toUpperCase() + log.log_type.slice(1);
                $row.append('<td><span class="simpli-log-type ' + typeClass + '">' + typeLabel + '</span></td>');
                
                // Action
                var actionClass = 'action-' + log.action;
                var actionLabel = log.action.charAt(0).toUpperCase() + log.action.slice(1);
                $row.append('<td><span class="simpli-action ' + actionClass + '">' + actionLabel + '</span></td>');
                
                // Object
                var objectHtml = log.object_title || '-';
                if (log.log_type === 'post' && log.object_id) {
                    objectHtml = '<a href="post.php?post=' + log.object_id + '&action=edit" target="_blank">' + 
                                 (log.object_title || 'Post #' + log.object_id) + '</a>';
                }
                ...
                if (log.user_id && log.user_id > 0) {
                    userHtml = '<a href="user-edit.php?user_id=' + log.user_id + '" target="_blank">' + 
                               log.user_name + '</a>';
                }
        ...
        function buildDetailsHtml(log) {
            ...
            if (log.old_value && log.new_value && log.old_value !== log.new_value) {
                ...
                html += '<span class="simpli-details-change">' + log.old_value + ' → ' + log.new_value + '</span>';
                html += '</div>';
            } else if (log.new_value) {
                html += '<div class="simpli-details-item">';
                html += '<span class="simpli-details-label">Version:</span> ';
                html += '<span class="simpli-details-value">' + log.new_value + '</span>';
                html += '</div>';
            }
            
            if (log.additional_data) {
                try {
                    var additional = typeof log.additional_data === 'string' ? 
                                    JSON.parse(log.additional_data) : log.additional_data;
                    
                    if (additional.title && additional.title.old && additional.title.new) {
                        html += '<div class="simpli-details-item">';
                    ...
                    if (additional.content) {
                        html += '<div class="simpli-details-item">';
                        html += '<span class="simpli-details-label">Content modified</span>';
                        html += '</div>';
                    }
                } catch(e) {
                    // Invalid JSON, skip
